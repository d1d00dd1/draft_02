...
function Visualizer({ stream }: VisualizerProps) {
    ...
    useEffect(() => {
        ...
        const sketch = (p: p5) => {
            ...
            const shouldThrottleClick = (x: number, y: number) => {
                const now = Date.now();
                if (now - lastClickTime < CLICK_RATE_LIMIT) {
                    pendingRipples.push({ x, y });
                    return true;
                }
                lastClickTime = now;
                return false;
            ...
            const addFloatingAscii = (x: number, y: number, life: number, vel: number, lift: number) => {
                if (floatingAscii.length >= 12) return;
                floatingAscii.push({
                    x,
                    y,
                    char: EMPTY_CHAR,
            ...
            (p as any).touchStarted = () => {
                if (shouldThrottleClick(p.mouseX, p.mouseY)) return false;
                startInteraction(p.mouseX, p.mouseY);
                addRipple(p.mouseX, p.mouseY, true);
                addFloatingAscii(p.mouseX, p.mouseY, 45, 6, 1.5);
                return false; 
            };
            ...
            p.mousePressed = () => {
                if (shouldThrottleClick(p.mouseX, p.mouseY)) return;
                startInteraction(p.mouseX, p.mouseY);
                addRipple(p.mouseX, p.mouseY, true);
                addFloatingAscii(p.mouseX, p.mouseY, 40, 5, 1.5);
            };
            ...
            p.mouseClicked = () => {
                ...
                updateGlitchMap(true);
                audioEngine.setChaos(1.0);
                addRipple(p.mouseX, p.mouseY, false);
            };
            ...
            function getColor(t: number, kick: number, chaos: number) {
                ...
                if (kick > 0.4) {
                    const pulse = Math.sin(t * 10 + chaos) * 0.15 + 0.85;
                    r *= pulse;
                    g *= pulse;
                    b *= pulse;
                }
                ...
                if (chaos > 0.6) {
                    ...
                    g = Math.max(0, Math.min(255, g - glitchShift * 0.5));
                ...
                try {
                    ...
                    if (!stutterActive) {
                        ...
                        const layers = [
                            ...
                            { type: 'petal', rings: 12, dens: 80, rad: 1.0, height: 0.4, k: 8, angle: Math.PI/2.2, width: 1.9, offset: Math.PI/6 },
                            { type: 'petal', rings: 14, dens: 90, rad: 1.4, height: 0.15, k: 10 + (mid*12), angle: Math.PI/1.8, width: 2.2, offset: Math.PI/8 },
                    ...
                    for (let i = 0; i < size; i++) {
                        ...
                        if (ripples.length > 0 && nearestRippleCache && nearestRippleCache[i] >= 0) {
                            ...
                            if (cachedIdx < ripples.length) {
                                const ripple = ripples[cachedIdx];
